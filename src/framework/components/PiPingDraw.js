...
class PiPingDraw extends Base{
  ...
  /**
   * 默认情况下只需传入需要更改的线条
   * color是在arr没有的情况下传入,用于控制所有的线条
   * @param arr
   * @param color
   */
  updata(arr,color){
    let temp = arr||this._arr;
    temp.forEach(a=>{
      ...
      if(tempSp){
        ...
        tempSp.alpha = 1;
        let tempColor = color||a.color;
        this._arr[index].color = tempColor;
        this.drawItem(this._arr[index],tempSp.getChildAt(0));
        this.addChildAt(tempSp,this.numChildren-1);
        tempSp = null;
  ...
  getChild(str){
    ...
    return tempSp;
    // return tempSp||this.nameArr[str];
  }
  ...
  drawItem(obj,tempSp){
    ...
    let r = obj.r!=undefined?obj.r:this.r;
    //分别为底图 灰色层图  高光层 遮罩
    let sh = tempSp||new createjs.Shape();
    this.drawPiPing(sh,obj.value,this.rw,r,'rgba(255,0,0,1)',obj.color);
    ...
    if(!tempSp){
      ...
      sp.name = obj.name;
      this.addChild(sp);
      // if(this.nameArr[name]) this.nameArr[name].push(sp);
      // else{
      //     this.nameArr[name] = [];
      //     this.nameArr[name].push(sp);
      // }
    }

    // let s2 = this.drawPiPing(new createjs.Shape(),obj.value,this.rw,r,'rgba(0,0,0,0)','#rgba(0,0,0)')
    // let sFil = this.drawPiPing(new createjs.Shape(),obj.value,this.rw/2,r,'rgba(0,0,0,0)',obj.color);
    // let maskS = this.drawPiPing(new createjs.Shape(),obj.value,this.rw,r,'rgba(0,0,0,0)','#rgba(0,0,0)')
    // s2.alpha = 0.45;
    // sFil.alpha = 0.65;
    //
    ...
    this.displayArr.length<1&&this.displayArr.push(sh);
        ...
        if(j==0){
          ...
          if($dataArr[i+1] == $dataArr[i+3]){
            ...
            if(H>=0&&V>0){
              ...
              if($r>0) $s.graphics.quadraticCurveTo($dataArr[i+2] + $hn,$dataArr[i+1] - $hn, $dataArr[i+2] + $hn, $dataArr[i+1] - $hn - $r);
              else{
                $s.graphics.lineTo($dataArr[i+2] + $hn,$dataArr[i+1] - $hn);
                $s.graphics.lineTo($dataArr[i+2] + $hn, $dataArr[i+1] - $hn - $r);
              }
            }
            //r-b
            if(H>=0&&V<=0){
              ...
              if($r>0) $s.graphics.quadraticCurveTo($dataArr[i+2] - $hn,$dataArr[i+1] - $hn, $dataArr[i+2] - $hn, $dataArr[i+1] + $hn + $r);
                ...
                $s.graphics.lineTo($dataArr[i+2] - $hn,$dataArr[i+1] - $hn, $dataArr[i+2] - $hn);
                $s.graphics.lineTo($dataArr[i+2] - $hn, $dataArr[i+1] + $hn + $r);
              }
            }
            //l-t
            ...
            if(H>=0&&V>0){
              ...
              if($r>0)$s.graphics.quadraticCurveTo($dataArr[i+2] + $hn,$dataArr[i+3] - $hn, $dataArr[i+2] - $hn-$r, $dataArr[i+3] - $hn);
              else {
                $s.graphics.lineTo($dataArr[i+2] + $hn,$dataArr[i+3] - $hn);
                $s.graphics.lineTo($dataArr[i+2] - $hn-$r, $dataArr[i+3] - $hn);
              }
            }
            //b-r
            if(H>=0&&V<=0){
          ...
          if(rArr[i+1] == rArr[i+3]){
            ...
            if(H>=0&&V<0){
              ...
              if($r>0) $s.graphics.quadraticCurveTo(rArr[i+2] - $hn,rArr[i+1] - $hn, rArr[i+2] - $hn, rArr[i+1] + $hn + $r);
              else{
                $s.graphics.lineTo(rArr[i+2] - $hn,rArr[i+1] - $hn);
                $s.graphics.lineTo(rArr[i+2] - $hn, rArr[i+1] + $hn + $r);
              }
            }
            if(H<0&&V<0){
              $s.graphics.lineTo(rArr[i+2] - $r - $hn,rArr[i+1] + $hn);
            ...
            if(H<0&&V>0){
              ...
              if($r>0) $s.graphics.quadraticCurveTo(rArr[i+2] + $hn,rArr[i+3] + $hn, rArr[i+2] + $hn + $r, rArr[i+3] + $hn);
              else{
                $s.graphics.lineTo(rArr[i+2] + $hn,rArr[i+3] + $hn);
                $s.graphics.lineTo(rArr[i+2] + $hn + $r, rArr[i+3] + $hn);
              }
            }
            if(H>0&&V>0){
              $s.graphics.lineTo(rArr[i+2] + $hn,rArr[i+3] + $hn + $r);
